...
class IBClient:
    ...
    def __init__(self, username: str, account: str, client_gateway_path: str = None, is_server_running: bool = True) -> None:
        ...
        if client_gateway_path is None:
            ...
            self.client_portal_folder: pathlib.Path = pathlib.Path(__file__).parents[1].joinpath("resources/clientportal.beta.gw").resolve()
    ...
    def _check_authentication_non_input(self) -> bool:
        ...
        # Log the Auth response.
        logging.debug("Check Non-User Auth Inital: {auth_resp}".format(auth_resp=auth_response))
    ...
    def _start_server(self) -> str:
        ...
        if self._operating_system == "win32":
            ...
            self.server_process = subprocess.Popen(
                args=IB_WEB_API_PROC, cwd=self.client_portal_folder, creationflags=subprocess.CREATE_NEW_CONSOLE
            ).pid
    ...
    def connect(self, start_server: bool = True, check_user_input: bool = True) -> bool:
        ...
        if check_user_input:
            ...
            {lin_brk}""".format(
                        lin_brk="-" * 80, url=self.login_gateway_path, proc_id=self.server_process
                    )
                )
            )
    ...
    def close_session(self) -> None:
        ...
        # Define the process.
        process = "TASKKILL /F /PID {proc_id} /T".format(proc_id=self.server_process)
    ...
    def _make_request(self, endpoint: str, req_type: str, headers: str = "json", params: dict = None, data: dict = None, json: dict = None) -> Dict:
        ...
        if response.ok:
            ...
            """.format(
                    resp_text=response.text, resp_url=response.url, resp_code=status_code, resp_json=data, resp_headers=response_headers
                )
            )
    ...
    def _fundamentals_financials(self, conid: str, financial_statement: str, period: str = "annual") -> Dict:
        ...
        # Build the arguments.
        params = {"type": financial_statement, "annual": period}
    ...
    def _data_analyst_forecast(self, conid: str) -> Dict:
        ...
        # Build the arguments.
        params = {"widgets": "analyst_forecast"}
    ...
    def symbol_search(self, symbol: str) -> Dict:
        ...
        content = self._make_request(endpoint=endpoint, req_type=req_type, json=payload)
    ...
    def run_scanner(self, instrument: str, scanner_type: str, location: str, size: str = "25", filters: List[dict] = None) -> Dict:
        ...
        req_type = "POST"
        payload = {"instrument": instrument, "type": scanner_type, "filter": filters, "location": location, "size": size}
